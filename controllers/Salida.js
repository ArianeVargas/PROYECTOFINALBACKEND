...
const httpSalida = {
  ...
  getSalidasId: async (req, res) => {
    try {
      const salida = await Salida.findById(req.params.id).populate("Pedido_id").populate("Usuario_id");
      if (!salida) {
  ...
  postSalida: async (req, res) => {
    try {
      const {Usuario_id, Pedido_id, Entregado, Total} = req.body;
      ...
      const nuevoSalida = new Salida({Usuario_id, Pedido_id, Total, Entregado, Numero});
